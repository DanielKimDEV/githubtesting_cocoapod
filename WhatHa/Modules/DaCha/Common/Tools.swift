...
//  WhatFilm
//
//  Created by Jason Kim on 11/09/2018.
...
extension Reactive where Base: UIScrollView {
    ...
    public var reachRight: Observable<Void> {
        let scrollView = self.base as UIScrollView
        return self.contentOffset.flatMap{ [weak scrollView] (contentOffset) -> Observable<Void> in
            guard let scrollView = scrollView else { return Observable.empty() }
            let visibleWidth = scrollView.frame.width - self.base.contentInset.left - scrollView.contentInset.right
...
extension TextStyle {
    ...
    var attributes: [String: AnyObject] {
        return [convertFromNSAttributedStringKey(NSAttributedString.Key.font): self.font, convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor): self.color]
    }
}
