...
public final class TVDetailsViewController: UIViewController, ReactiveDisposable {
    
    // MARK: - Properties
    
    let disposeBag: DisposeBag = DisposeBag()
    var viewModel: TVDetailsViewModel?
    ...
    override public func viewDidLoad() {
        super.viewDidLoad()
        log?.debug("TV Details is View DId Load!")
        self.setupUI()
        self.setupCollectionViews()
        log?.debug("get view model data \(self.viewModel?.credits.debug())")
        log?.debug("get view model data \(self.viewModel?.tvDetail.debug())")
    ...
    override public func viewDidLayoutSubviews() {
        ...
        // Adjust scrollview insets based on tv title
    ...
    fileprivate func populate(forFilmDetail filmDetail: TVDetail) {
    ...
    public func prePopulate(forTV tv: TV) {
        if let posterPath = tv.posterPath { self.blurredImageView.setImage(fromTMDbPath: posterPath, withSize: .medium, animatedOnce: true) }
        self.filmTitleLabel.text = tv.fullTitle.uppercased()
        self.filmOverviewLabel.text = tv.overview
    }
    
    // MARK: - Reactive setup
    
    fileprivate func setupBindings(forViewModel viewModel: TVDetailsViewModel) {
        ...
        self.backgroundImagePath = viewModel.tvDetail.map { (filmDetail) -> ImagePath? in
    ...
    override public func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let personViewController = segue.destination as? TVPersonViewController,
...
extension TVDetailsViewController: SegueReachable {
    
    // MARK: - SegueReachable
    
    static var segueIdentifier: String { return PushTVDetailsSegue.identifier }
}

// MARK: -

extension TVDetailsViewController: UICollectionViewDelegate {
    ...
    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        self.performSegue(withIdentifier: TVPersonViewController.segueIdentifier, sender: sender)
    }
}

// MARK: -

extension TVDetailsViewController: UICollectionViewDelegateFlowLayout {
...
extension TVDetailsViewController: PersonFromCellTransitionable { }
